...
#   - openvpn config file must be named "client" and reside in /home/amnesia/Persistent/vpn
...
# check for tcp config in client
if [[ ! $(cat /home/amnesia/Persistent/vpn/client.ovpn | grep "proto tcp") ]]
then
...
# check that no "socks-proxy" directive is in client
if [[ $(cat /home/amnesia/Persistent/vpn/client.ovpn  | grep "socks-proxy") ]]
then
        echo "This script does not allow to have "socks-proxy" in client because it will be added by this scrpt"
  read -n1 -r -p "Press any key to exit..." key
  exit 1
fi
...
# clean client from DOS line breaks if necessary
if `grep -r $'\r' /home/amnesia/Persistent/vpn/client.ovpn >/dev/null`
then
  unset votcfgperm
  votcfgperm=`stat -c%a /home/amnesia/Persistent/vpn/client.ovpn `
  tr -d $'\r' < /home/amnesia/Persistent/vpn/client.ovpn  >/tmp/client.ovpn  && mv /tmp/client.ovpn  /home/amnesia/Persistent/vpn/
  chmod "$votcfgperm" /home/amnesia/Persistent/vpn/client.ovpn 
  unset votcfgperm
fi

# add socks proxy option to OpenVPN config and save it to /etc/openvpn/
awk '/^remote /{print $0 RS "socks-proxy 127.0.0.1 9053";next}1' /home/amnesia/Persistent/vpn/client.ovpn  > /etc/openvpn/client.ovpn 

# add parameter "up-delay" to client to avoid the SIGHUP/SIGUSR1 after establishing connection
grep "up-delay" /etc/openvpn/client.ovpn  >/dev/null || echo "up-delay" >> /etc/openvpn/client.ovpn 

# add parameters to client to better handle unstable Tor
grep "connect-retry-max" /etc/openvpn/client.ovpn  >/dev/null || echo "connect-retry-max 1" >> /etc/openvpn/client.ovpn 
# grep "connect-retry" /etc/openvpn/client.ovpn  >/dev/null || echo "connect-retry 20" >> /etc/openvpn/client.ovpn 
# grep "connect-timeout" /etc/openvpn/client.ovpn  >/dev/null || echo "connect-timeout 120" >> /etc/openvpn/client.ovpn 

# add settings to drop privileges by running as user/group "openvpn"
grep "user openvpn" /etc/openvpn/client.ovpn  >/dev/null || echo "user openvpn" >> /etc/openvpn/client.ovpn 
grep "group openvpn" /etc/openvpn/client.ovpn  >/dev/null || echo "group openvpn" >> /etc/openvpn/client.ovpn 

# OpenVPN debug output
# echo "verb 11" >> /etc/openvpn/client.ovpn 

# put OpenVPN in a chroot jail
mkdir -p /tmp/openvpn/jail/tmp
if [[ ! $(cat /etc/openvpn/client.ovpn | grep "chroot ") ]]
then
  echo "chroot /tmp/openvpn/jail" >> /etc/openvpn/client.ovpn 
fi
